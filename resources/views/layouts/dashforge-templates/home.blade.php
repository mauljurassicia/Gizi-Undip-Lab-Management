                ...
                <div>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Analisis Data Laboratorium</li>
                        </ol>
                    </nav>
                    <h4 class="mg-b-0 tx-spacing--1">Selamat Datang di Dashboard</h4>
                </div>
                ...
                <div class="col-lg-8 col-xl-9">
                    <div class="card h-100 ">
                        <div
                            ...
                            <div>
                                <h6 class="mg-b-5">Tingkat Kunjungan dan Peminjaman Alat Laboratorium</h6>
                                <p class="tx-12 tx-color-03 mg-b-0">
                                    Jumlah kunjungan dan peminjaman alat laboratorium selama jangka waktu seminggu terakhir
                                </p>
                            </div>
                            <div class="btn-group mg-t-20 mg-sm-t-0">
                                <button class="btn btn-xs btn-white btn-uppercase date-btn"
                                    onclick="changeDate('day', event)">Day</button>
                                <button class="btn btn-xs btn-white btn-uppercase date-btn"
                                    onclick="changeDate('week', event)">Week</button>
                                <button class="btn btn-xs btn-white btn-uppercase active date-btn"
                                    onclick="changeDate('month' , event)">Month</button>
                            </div><!-- btn-group -->
                        </div><!-- card-header -->
                        <div class="card-body pd-lg-25">
                            <div class="row align-items-center h-100">
                                <div class="col-12">
                                    <div class="chart-six"><canvas id="chartLine1"></canvas></div>
                                </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6 col-lg-12 mg-t-30 mg-sm-t-0 mg-lg-t-30">
                                            ...
                                            <div class="d-flex justify-content-between mg-b-5">
                                                ...
                                                <h5 class="tx-normal tx-rubik tx-color-03 mg-b-0"><small>250,000</small>
                                                </h5>
                                            </div>
                                            <div class="progress ht-4 mg-b-0 op-5">
                                                <div class="progress-bar bg-orange wd-45p" role="progressbar"
                ...
                <div class="col-md-6 col-lg-4 col-xl-3 mg-t-10 mg-lg-t-0">
                    <div class="card">
                        <div class="card-header">
                            <h6 class="mg-b-0">Komposisi Pengguna</h6>
                        </div><!-- card-header -->
                        <div class="card-body pd-lg-25">
                        ...
                        <div class="card-footer pd-20">
                            <div class="row">
                                <div class="col-6">
                                    <p class="tx-10 tx-uppercase tx-medium tx-color-03 tx-spacing-1 tx-nowrap mg-b-5">Dosen
                                    </p>
                                    <div class="d-flex align-items-center">
                                        <div class="wd-10 ht-10 rounded-circle bg-pink mg-r-5"></div>
                                        <h6 class="tx-normal tx-rubik mg-b-0">{{ $teacherCount }} <small
                                                class="tx-color-04">{{ getPercentage($teacherCount, $totalUserCount) }}%</small>
                                        </h6>
                                    </div>
                                </div><!-- col -->
                                <div class="col-6">
                                    <p class="tx-10 tx-uppercase tx-medium tx-color-03 tx-spacing-1 mg-b-5">Pengunjung</p>
                                    <div class="d-flex align-items-center">
                                        <div class="wd-10 ht-10 rounded-circle bg-primary mg-r-5"></div>
                                        <h6 class="tx-normal tx-rubik mg-b-0">{{ $guestCount }} <small
                                                class="tx-color-04">{{ getPercentage($guestCount, $totalUserCount) }}%</small>
                                        </h6>
                                    </div>
                                </div><!-- col -->
                                <div class="col-6 mg-t-20">
                                    <p class="tx-10 tx-uppercase tx-medium tx-color-03 tx-spacing-1 mg-b-5">Mahasiswa</p>
                                    <div class="d-flex align-items-center">
                                        <div class="wd-10 ht-10 rounded-circle bg-teal mg-r-5"></div>
                                        <h6 class="tx-normal tx-rubik mg-b-0">{{ $studentCount }} <small
                                                class="tx-color-04">{{ getPercentage($studentCount, $totalUserCount) }}%</small>
                                        </h6>
                                    </div>
                                </div><!-- col -->
                                <div class="col-6 mg-t-20">
                                    <p class="tx-10 tx-uppercase tx-medium tx-color-03 tx-spacing-1 mg-b-5">Laborat
                                    </p>
                                    <div class="d-flex align-items-center">
                                        <div class="wd-10 ht-10 rounded-circle bg-orange mg-r-5"></div>
                                        <h6 class="tx-normal tx-rubik mg-b-0">{{ $laborantCount }} <small
                                                class="tx-color-04">{{ getPercentage($laborantCount, $totalUserCount) }}%</small>
                                        </h6>
                                    </div>
                                </div><!-- col -->
                            </div><!-- row -->
...
@section('scripts')
    ...
    <script>
        ...
        function changeDate(type, event) {
        ...
        function updateChart() {
            ...
            if (dateType === 'day') {
                data = @json($scheduleDates->pluck('count')->toArray());
                dataBorrow = @json($borrowingDates->pluck('count')->toArray());
                labels = @json($scheduleDates->pluck('date')->toArray());
                labelsBorrow = @json($borrowingDates->pluck('date')->toArray());
            } else if (dateType === 'week') {
                data = @json($scheduleWeek->pluck('count')->toArray());
                dataBorrow = @json($borrowingWeek->pluck('count')->toArray());
                labels = @json($scheduleWeek->pluck('day')->toArray());
                labelsBorrow = @json($borrowingWeek->pluck('day')->toArray());
            } else if (dateType === 'month') {
                data = @json($scheduleMonths->pluck('count')->toArray());
                dataBorrow = @json($borrowingMonths->pluck('count')->toArray());
                labels = @json($scheduleMonths->pluck('month')->toArray());
                labelsBorrow = @json($borrowingMonths->pluck('month')->toArray());
            }
        ...
        $(function() {
            ...
            /** PIE CHART **/
            var datapie = {
                labels: ['Dosen/Pengajar', 'Pengunjung', 'Mahasiswa', 'Laborat'],
                datasets: [{
                    data: [{{ getPercentage($teacherCount, $totalUserCount) }},
                        {{ getPercentage($guestCount, $totalUserCount) }},
                        {{ getPercentage($studentCount, $totalUserCount) }},
                        {{ getPercentage($laborantCount, $totalUserCount) }}
                    ],
